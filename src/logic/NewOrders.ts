...
export async function GetOrders(
  ...
  try {
    ...
    console.log(
      `Server responded with an error when getting new orders, ${err}`
    );
    return [];
  }
}

function ParseSoldOrdersHtml(root: cheerio.Root, Type: OrderType): INewOrder[] {
  // Please replace with an JSON Api Call if one it exists.
  ...
  root("tr", TABLE).each(function (i, item) {
    ...
    const OrderId: number = Number(
      root(".sales-history__product-id", item)
        ?.attr("href")
        ?.match(/(?<=oid=)\d+/)
    );
    ...
    if (Type === OrderType.All) {
      const order_type = parseInt(
        root("span.status", item).attr().class.split(" ")[1].replace(/\D/g, "")
      );
      type = order_type;
    }
